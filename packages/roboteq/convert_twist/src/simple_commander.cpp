...
int main(int argc, char **argv) {
    ...
    trajBuilder.set_alpha_max(1);
    trajBuilder.set_speed_max(0.4);
    ...
    g_end_pose.pose.position.x = 0.0; //set goal coordinates
    ...
    while (ros::ok()) {
        ...
        //hacky open loop thing
        
        
        if (asdf==1){
            g_end_pose.pose.position.x = 4.7244;
            g_end_pose.pose.position.y = 0;
        }
        else if (asdf == 2){
            g_end_pose.pose.position.x = 0;
            g_end_pose.pose.position.y = -13;
        }
        else if(asdf ==3){
            g_end_pose.pose.position.x = 0;
            g_end_pose.pose.position.y = -3;
        }else{
            ROS_INFO("lol");
            break;
        }
    ...
    //     std::cout<<"x: ";
    //     std::cin >> g_end_pose.pose.position.x;
    //     std::cout<<"y: ";
    //     std::cin >> g_end_pose.pose.position.y;

        ROS_INFO("going to %f, %f",g_end_pose.pose.position.x,  g_end_pose.pose.position.y);
        ROS_INFO("from %f, %f", g_start_pose.pose.position.x, g_start_pose.pose.position.y);
        trajBuilder.build_point_and_go_traj(g_start_pose, g_end_pose, vec_of_states);
    ...
    // ros::Rate loop_timer(1/0.01); //create a ros object from the ros “Rate” class; set 100Hz rate     
    // double timer=0.0;
    ...
    //   loop_timer.sleep();
    // }
    // twist_cmd.angular.z=0.5; //command to move forward
    // while(ros::ok()) {
    //       twist_commander.publish(twist_cmd);
    //       //timer+=sample_dt;
    //       loop_timer.sleep();
