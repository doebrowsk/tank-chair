...
void SteeringController::lin_steering_algorithm() {
    ...
    // debug info
    // ROS_INFO("--------------------------------------------------------------------------------");
    // ROS_INFO("des_state_phi, odom_phi, heading_err = %f, %f, %f", des_state_phi_,odom_phi_,heading_err);
    //ROS_INFO("odom_x_, odom_y_ = %f, %f", odom_x_,odom_y_);
    // ROS_INFO("des_state_x_, des_state_y_ = %f, %f", des_state_x_,des_state_y_);
    // ROS_INFO("lateral err, trip dist err = %f, %f",lateral_err,trip_dist_err);
    // ROS_INFO("odom_omega_, des_state_omega_ = %f, %f", odom_omega_,des_state_omega_);
    // ROS_INFO("odom_vel_, des_state_vel_ = %f, %f", odom_vel_,des_state_vel_);
    // ROS_INFO("controller_speed %f", controller_speed);
    // ROS_INFO("controller_omega %f", controller_omega);
